...
// game colors
let boardColor = "#000000", headColor = "yellow", bodyColor = "lightgreen";
...
function changeLvl() {
    ...
    let defaultSpeed = score < 4;
    let slowSpeed = score > 3 && score < 8;
    let mediumSpeed = score > 7 && score < 12;
    let fastSpeed = score > 11 && score < 17;
    let hardSpeed = score > 16;
    if (defaultSpeed) {
        currentLvl = levels.default
    };
    if (slowSpeed) {
        currentLvl = levels.first
    };
    if (mediumSpeed) {
        currentLvl = levels.second
    };
    if (fastSpeed) {
        currentLvl = levels.third
    };
    if (hardSpeed) {
        currentLvl = levels.fourth
    };
...
function setDirection(event) {
    ...
    if(
        ...
        if(!gameStarted) {
            gameStarted = true;
            gameLoop = setInterval(frame, currentLvl);
        }
...
// numbers of vertical/horizontal squares
...
function gameOver() {
    ...
    highScoreEl.innerHTML = `Рекорд: ${highScore}`;
...
function restartGame() {
    // reset snake lengthand position
    snake = [
        {x : 1, y : 0}, //head 
        {x : 0, y : 0}, //body
    ];
